docker-image: hermes.Dockerfile
...
restore-keys: hermes.config
	MNEMONIC="$$(cat $(KEYFILE))"; \
	echo $$MNEMONIC | sha1sum ; \
	docker run --rm -v$$PWD:/config hermes -c /config/hermes.config keys restore agoricdev-6 -p "m/44'/564'/0'/0/0" -m "$$MNEMONIC"; \
	docker run --rm -v$$PWD:/config hermes -c /config/hermes.config keys restore cosmoshub-testnet -m "$$MNEMONIC"
